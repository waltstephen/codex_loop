...
class FeishuTokenManager:
    ...
    def get_token(self) -> str | None:
        ...
        if self._token and now < self._expires_at - 30:
            return self._token
        payload = json.dumps({"app_id": self.app_id, "app_secret": self.app_secret}, ensure_ascii=True).encode("utf-8")
        ...
        parsed = _perform_json_request(req, timeout_seconds=self.timeout_seconds, on_error=self.on_error, label="feishu auth")
...
class FeishuCommandPoller:
    ...
    def _run(self) -> None:
        while not self._stop_event.is_set():
            for item in self._fetch_messages():
                ...
                if not text:
                    continue
                parsed = parse_control_text(text=text, plain_text_kind=self.plain_text_kind)
                if parsed is None:
                    continue
                try:
                    self.on_command(FeishuCommand(kind=parsed.kind, text=parsed.text))
    ...
    def _fetch_messages(self) -> list[dict[str, Any]]:
        ...
        for item in rows:
            if str(item.get("message_id") or "") == self._last_message_id:
                break
            fresh.append(item)
...
def format_feishu_event_message(event: dict[str, Any]) -> str:
    ...
    if event_type == "plan.finalized":
        return (
            ...
            f"summary={str(event.get('summary', ''))}\n"
            f"next={str(event.get('suggested_next_objective', ''))}"
        )
    if event_type == "loop.completed":
        return (
            f"[autoloop] completed\n"
            f"success={event.get('success')}\n"
            f"stop_reason={str(event.get('stop_reason', '')).strip()[:500]}"
        )
    return ""
